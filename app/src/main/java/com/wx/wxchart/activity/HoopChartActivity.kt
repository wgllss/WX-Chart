...
class HoopChartActivity : ComponentActivity() {
            ...
            setContent {
                WXChart {
                    baseUI2({ paddingvalues ->
                        HoopChart(paddingvalues, viewModel)
                    }, {
                        finish()
                    })
                }
...
@Composable
fun HoopChart(innerPadding: PaddingValues = PaddingValues(100.dp), viewModel: SampleViewModel = SampleViewModel().apply { setData() }) {
    var touchPieData by remember { mutableStateOf(Triple(-1, 0f, 0f)) }
    val textMeasurer = rememberTextMeasurer()
    val chatModel by viewModel.chatHoopModel.observeAsState()
    chatModel?.let {
        Column(
            modifier = Modifier
